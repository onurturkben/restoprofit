...
def inject_settings():
    # Logo ve site adını buradan yönetebiliriz, ancak şimdilik sadece kullanıcıyı enjekte edelim
    # Daha sonra Ayarlar tablosu eklenebilir.
...
def admin_panel():
    try:
        ...
        # *** BURASI DÜZELTİLDİ ***
        # Reçeteleri alırken Urun ve Hammadde tablolarına join yapıyoruz.
...
def reports():
    try:
        ...
        gruplar_db = db.session.query(Urun.kategori_grubu).distinct().all().order_by(Urun.kategori_grubu).all()
